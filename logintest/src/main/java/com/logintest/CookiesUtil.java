...
public class CookiesUtil {
    ...
    public static void setLoginUser(HttpServletResponse response, CookiesUser cookiesUser) {
        ...
        int id = cookiesUser.getId();
        String username = cookiesUser.getUsername();
        try{
        ...
        // cookies : ID | 用户名 | 登录状态
        cookiesValue.append(id).append("|").append(username).append("|").append(cookiesUser.getLoginStatus());
    ...
    public static CookiesUser getLoginUser(HttpServletRequest request) {
        ...
        cookiesUser.setId(Integer.parseInt(array[0]));
        try {
            cookiesUser.setUsername(URLDecoder.decode(array[1], "UTF-8"));
        } catch(UnsupportedEncodingException e) {
            cookiesUser.setUsername(array[1]);
        }
